...
class hb_liked_portfolio_widget extends WP_Widget {
	...
	function __construct() {
		...
		$control_ops = array ();
		/* Create the widget. */
		parent::__construct( 'hb_liked_portfolio_widget', __('[HB-Themes] Most Liked Portfolio Widget','hbthemes'), $widget_ops, $control_ops );
	}
	...
	function widget( $args, $instance ) {
		...
		if ( $portfolio_items->have_posts() ) : while ( $portfolio_items->have_posts() ) : $portfolio_items->the_post();
			...
			if ( $thumb ) {
				$image = highend_resize( $thumb, 250, 250 );
				$counter--;
				if ($counter>=0) {
					echo '<li><a href="'.$perma.'" title="'.get_the_title().'" rel="tooltip">';
					echo '<img src="'.$image['url'].'" />';
					echo '<span class="widget-like-count"><i class="hb-moon-heart-3"></i>'. number_format($likes) .'</span>';
					echo '</a></li>';			
				}
			}
			endwhile; endif;
