...
if( !function_exists('vp_ajax_wrapper') )
{
	function vp_ajax_wrapper()
	{
		...
		else
		{
			$result['data']    = '';
			$result['status']  = false;
			$result['message'] = __("Unauthorized function", 'hbthemes');		
		}
...
if( !function_exists('vp_sg_enqueue') )
{
	function vp_sg_enqueue()
	{
		if( VP_ShortcodeGenerator::pool_can_output() )
		{
			...
			wp_register_script( 'vp-sg-dummy', VP_PUBLIC_URL . '/js/dummy.js', array(), HB_THEME_VERSION, false );
...
if( !function_exists('vp_post_dummy_editor') )
{
	function vp_post_dummy_editor()
	{
		...
		if( $dummy )
		{
			$callback = function(){ return "tinymce"; };

			echo '<div style="display: none">';
			add_filter( 'wp_default_editor', $callback );
