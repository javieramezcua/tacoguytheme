<?php
/*
 * Plugin Name: Portfolio Widget Random
 * Plugin URI: http://www.hb-themes.com
 * Description: A widget that displays random Portfolio posts
 * Version: 1.0
 * Author: HB-Themes
 ...
 */
add_action( 'widgets_init', 'hb_portfolio_widgets_rand' );

/*
 * Register widget.
 */
function hb_portfolio_widgets_rand() {
	register_widget( 'HB_Portfolio_Widget_Rand' );
}

/*
 * Widget class.
 */
class hb_portfolio_widget_rand extends WP_Widget {
	...
	function __construct() {
		...
		/* Widget settings. */
		$widget_ops = array( 'classname' => 'portfolio-widget-rand', 'description' => __('A widget that displays random portfolio posts.', 'hbthemes') );
		$control_ops = array ();
		/* Create the widget. */
		parent::__construct( 'hb_portfolio_widget_rand', __('[HB-Themes] Portfolio Grid (Random)','hbthemes'), $widget_ops, $control_ops );
	}
	...
	function widget( $args, $instance ) {
		...
		/* Display Dribbble */
			...
			if ( $category ) {
				$loop = new WP_Query( array( 'post_type' => 'portfolio' , 'orderby' => 'rand', 'order' => 'ASC', 'posts_per_page' => $number, 'tax_query' => array( array( 'taxonomy' => 'portfolio_categories', 'field' => 'id', 'terms' => $category ) ) ) );
			} else {
				$loop = new WP_Query( array( 'post_type' => 'portfolio' , 'orderby' => 'rand', 'order' => 'ASC', 'posts_per_page' => $number ) );
			}
			...
			if ( $thumb ) {
				...
				$size  = apply_filters(
					'highend_portfolio_widget_image_size',
					array(
						'width' => 250,
						'height' => 250,
						'crop' => true,
				...
				$image = highend_resize( $thumb, $size['width'], $size['height'], $size['crop'] );
